...
describe('DeskController', () => {
    ...
    const request = { user: { sub: 'user-1', email: 'a@b.com', role: 'Employee', resortId: 'resort-1' } } as any;
    ...
    it('delegates claim to the service using the authenticated user id', async () => {
        service.claim.mockResolvedValue({ ...conversation, status: ConversationStatus.HUMAN });
    ...
    it('delegates close to the service', async () => {
        service.close.mockResolvedValue({ ...conversation, status: ConversationStatus.CLOSED });
