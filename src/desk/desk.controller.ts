import { BadRequestException, Body, Controller, Get, Headers, Param, Patch, Post, Req, UseGuards, UsePipes, ValidationPipe } from '@nestjs/common';
...
export class DeskController {
    ...
    @ApiHeader({
        ...
        required: false,
        description: 'Client-generated key (e.g. a UUID per compose action) to prevent a duplicate WhatsApp send if the request is retried after a network error.',
    })
    async reply(
        @Param('resortId') resortId: string,
        @Param('id') id: string,
        ...
        if (idempotencyKey && idempotencyKey.length > IDEMPOTENCY_KEY_MAX_LENGTH) {
            throw new BadRequestException(`Idempotency-Key header must not exceed ${IDEMPOTENCY_KEY_MAX_LENGTH} characters.`);
        }

        const message = await this.deskService.replyAsEmployee(resortId, id, request.user.sub, dto.body, idempotencyKey);
        return MessageResponseDto.fromEntity(message);
    }

    @Post('conversations/:id/messages/:messageId/retry')
    @ApiOperation({ summary: 'Retry delivering a message that permanently failed to send — its owner or employees may do this' })
    @ApiParam({ name: 'resortId', type: String })
    @ApiParam({ name: 'id', type: String })
    ...
    async retryMessage(
        ...
        const message = await this.deskService.retryMessageDelivery(resortId, id, messageId);
        return MessageResponseDto.fromEntity(message);
    }
}
