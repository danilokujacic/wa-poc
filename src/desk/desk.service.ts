...
export class DeskService {
    ...
    async recordMessage(params: RecordMessageParams): Promise<Message> {
        ...
        const isOutOfOrder = await this.checkAndUpdateOrdering(conversation.id, sentAt);
        ...
        this.conversationRepository.updateLastMessageSentAt(conversation.id, sentAt).catch((error) => {
            this.logger.warn({ traceId, conversationId: conversation.id }, `Failed to update lastMessageSentAt: ${error}`);
        });

        // Guests never have a "delivery" to track — they're the ones who sent it to us.
        const deliveryStatus = params.sender === MessageSenderType.GUEST ? null : MessageDeliveryStatus.PENDING;
        ...
        this.logger.info(
            { traceId: traceId ?? message.id, messageId: message.id, conversationId: conversation.id, resortId: params.resortId, sender: params.sender, deliveryStatus },
            'Message recorded',
        );
        ...
        if (isOutOfOrder) {
            ...
            this.logger.warn(
                { traceId: traceId ?? message.id, messageId: message.id, conversationId: conversation.id },
        ...
        if (deliveryStatus === MessageDeliveryStatus.PENDING && params.phoneNumberId) {
     ...
     */
    private async checkAndUpdateOrdering(conversationId: string, sentAt: Date): Promise<boolean> {
    ...
    async replyAsEmployee(
        ...
        if (idempotencyKey) {
            const existing = await this.claimIdempotencyKey(conversationId, idempotencyKey);
            if (existing) {
                return existing;
            }
        }
        ...
        this.logger.info({ traceId, conversationId, resortId, userId }, 'Employee reply requested');
        try {
            ...
            if (idempotencyKey) {
                ...
                await this.redis.del(this.idempotencyRedisKey(conversationId, idempotencyKey));
            }
            throw error;
        }
    }
     ...
     */
    private async claimIdempotencyKey(conversationId: string, idempotencyKey: string): Promise<Message | null> {
        const key = this.idempotencyRedisKey(conversationId, idempotencyKey);
        const claimed = await this.redis.set(key, 'pending', 'EX', IDEMPOTENCY_KEY_TTL_SECONDS, 'NX');
        if (claimed) {
            return null;
        }
        ...
        if (stored && stored !== 'pending') {
            const existing = await this.messageRepository.findOneForConversation(conversationId, stored);
            if (existing) {
                this.logger.info(
        ...
        throw new ConflictException('A reply with this idempotency key is already being processed.');
    }

    private idempotencyRedisKey(conversationId: string, idempotencyKey: string): string {
     ...
     */
    async retryMessageDelivery(resortId: string, conversationId: string, messageId: string): Promise<Message> {
        ...
        this.logger.info({ traceId, conversationId, resortId }, 'Manual delivery retry requested');

        await this.findConversationOrThrow(resortId, conversationId);

        const message = await this.messageRepository.findOneForConversation(conversationId, messageId);
        if (!message) {
            throw new NotFoundException(`Message with id ${messageId} not found in conversation ${conversationId}`);
        }

        if (message.sender === MessageSenderType.GUEST) {
            throw new BadRequestException('Guest messages were never sent by us — there is nothing to retry.');
        }
        ...
        if (claim.affected === 0) {
            this.logger.info({ traceId, conversationId }, 'Retry rejected — message is no longer eligible (lost the claim or not Failed)');
            throw new BadRequestException('Message is no longer eligible for retry — it may already be retrying.');
        }
        ...
        try {
            ...
            this.logger.error({ traceId, conversationId }, `Retry enqueue failed, reverting claim to Failed: ${error}`);
            await this.messageRepository.update(messageId, { deliveryStatus: MessageDeliveryStatus.FAILED });
            this.deskGateway.emitMessageStatusUpdated(resortId, {
                conversationId,
                messageId: message.id,
