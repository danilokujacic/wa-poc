...
import { MessageDeliveryStatus, MessageSenderType } from '../entity/message.entity';

describe('DeskService', () => {
    let service: DeskService;
    let conversationRepository: { findOrCreate: jest.Mock; save: jest.Mock; findOne: jest.Mock; findAllForResort: jest.Mock; findForResort: jest.Mock; isHumanHandled: jest.Mock; updateLastMessageSentAt: jest.Mock };
    let messageRepository: { create: jest.Mock; save: jest.Mock; findAllForConversation: jest.Mock; findOneForConversation: jest.Mock; update: jest.Mock };
    let resortRepository: { findOneBy: jest.Mock };
    let deskGateway: { emitNewMessage: jest.Mock; emitMessageStatusUpdated: jest.Mock };
    ...
    it('marks AI/employee messages "Pending" and enqueues them for durable delivery when a phoneNumberId is given', async () => {
        ...
        expect(messageRepository.create).toHaveBeenCalledWith(
            expect.objectContaining({ deliveryStatus: MessageDeliveryStatus.PENDING }),
        );
    ...
    it('handles a desk.message.received event by enqueuing it as a guest message', async () => {
        await service.handleMessageReceived({ resortId: 'resort-1', guestPhoneNumber: '38269280401', body: 'Hello', sentAt: '2026-01-01T10:00:00.000Z', traceId: 'wamid.123' });
    ...
    it('handles a desk.message.ai-replied event by enqueuing it as an AI message', async () => {
        await service.handleAiReplied({ resortId: 'resort-1', guestPhoneNumber: '38269280401', body: 'How can I help?', sentAt: '2026-01-01T10:00:00.000Z', phoneNumberId: 'phone-id-1', traceId: 'trace-1' });
    ...
    it('records an employee reply as "Pending" and durably enqueues its WhatsApp delivery', async () => {
        ...
        const message = await service.replyAsEmployee('resort-1', 'conv-1', 'user-1', 'On it!');
    ...
    describe('replyAsEmployee idempotency key', () => {
        ...
        it('claims the key via SET NX and records the reply as normal on first use', async () => {
            ...
            const message = await service.replyAsEmployee('resort-1', 'conv-1', 'user-1', 'On it!', 'key-1');

            expect(redis.set).toHaveBeenCalledWith('desk:idempotency:conv-1:key-1', 'pending', 'EX', 24 * 3600, 'NX');
            expect(redis.set).toHaveBeenCalledWith('desk:idempotency:conv-1:key-1', message.id, 'EX', 24 * 3600);
        ...
        it('returns the original message instead of creating a duplicate when the key was already used', async () => {
            ...
            messageRepository.findOneForConversation.mockResolvedValue({ id: 'existing-message-id', body: 'On it!' });
        ...
        it('rejects with a conflict if the key is claimed but still mid-flight (no message recorded yet)', async () => {
            ...
            await expect(service.replyAsEmployee('resort-1', 'conv-1', 'user-1', 'On it!', 'key-1')).rejects.toThrow(ConflictException);
            expect(messageRepository.save).not.toHaveBeenCalled();
        });
        ...
        it('releases the claim if recording the message throws, so a genuine retry is not blocked for the full TTL', async () => {
            ...
            await expect(service.replyAsEmployee('resort-1', 'conv-1', 'user-1', 'On it!', 'key-1')).rejects.toThrow('DB unavailable');
        ...
        it('does not touch Redis idempotency keys at all when no key is provided', async () => {
            ...
            expect(redis.set).not.toHaveBeenCalledWith(expect.stringContaining('desk:idempotency:'), expect.anything(), expect.anything(), expect.anything());
        });
    });
    ...
    describe('retryMessageDelivery', () => {
        ...
        it('re-enqueues a failed message unchanged and flips it back to Pending', async () => {
            ...
            messageRepository.findOneForConversation.mockResolvedValue({ ...failedMessage });
            resortRepository.findOneBy.mockResolvedValue({ id: 'resort-1', phoneNumber: '1211777188687734' });

            const result = await service.retryMessageDelivery('resort-1', 'conv-1', 'message-1');
            ...
            expect(deskGateway.emitMessageStatusUpdated).toHaveBeenCalledWith('resort-1', {
                conversationId: 'conv-1',
                messageId: 'message-1',
                deliveryStatus: MessageDeliveryStatus.PENDING,
            });
        ...
        it('throws when the message does not exist in that conversation', async () => {
            ...
            await expect(service.retryMessageDelivery('resort-1', 'conv-1', 'missing')).rejects.toThrow(NotFoundException);
            expect(waSendProducer.enqueue).not.toHaveBeenCalled();
        });

        it('rejects retrying a guest message', async () => {
            conversationRepository.findForResort.mockResolvedValue(conversation);
            messageRepository.findOneForConversation.mockResolvedValue({ ...failedMessage, sender: MessageSenderType.GUEST });

            await expect(service.retryMessageDelivery('resort-1', 'conv-1', 'message-1')).rejects.toThrow(BadRequestException);
            expect(waSendProducer.enqueue).not.toHaveBeenCalled();
        });

        it('rejects retrying a message that is not Failed, to avoid double-sending', async () => {
            conversationRepository.findForResort.mockResolvedValue(conversation);
            messageRepository.findOneForConversation.mockResolvedValue({ ...failedMessage, deliveryStatus: MessageDeliveryStatus.PENDING });
        ...
        it('reverts the claim back to Failed if enqueueing the retry itself fails, instead of leaving it stuck Pending', async () => {
            ...
            await expect(service.retryMessageDelivery('resort-1', 'conv-1', 'message-1')).rejects.toThrow('Redis unavailable');
            ...
            expect(messageRepository.update).toHaveBeenNthCalledWith(2, 'message-1', { deliveryStatus: MessageDeliveryStatus.FAILED });
            expect(deskGateway.emitMessageStatusUpdated).toHaveBeenNthCalledWith(1, 'resort-1', {
                conversationId: 'conv-1',
                messageId: 'message-1',
                deliveryStatus: MessageDeliveryStatus.PENDING,
            });
            expect(deskGateway.emitMessageStatusUpdated).toHaveBeenNthCalledWith(2, 'resort-1', {
                conversationId: 'conv-1',
                messageId: 'message-1',
                deliveryStatus: MessageDeliveryStatus.FAILED,
            });
