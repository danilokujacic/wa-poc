...
export class WhatsappService {
  ...
  async processIncoming(body: WhatsappWebhookPayload): Promise<void> {
    ...
    if (message.type !== 'text') {
      this.logger.info(
        ...
        'Received a non-text message; replying with the unsupported-type notice',
      );
      await this.sendText(
        '1211777188687734',
        '38269280401',
    ...
    try {
      ...
      this.logger.error(
        { traceId, conversationKey },
        `Error adding message to queue: ${error}`,
      );
    }
  }
  ...
  private async recordInboundMessageForDesk(
    ...
    try {
      ...
      this.logger.error(
        { traceId },
        `Error recording inbound message for desk: ${error}`,
      );
    }
  }
  ...
  async sendText(
    ...
    if (!res.ok) {
      ...
      throw new WhatsappSendError(
        res.status,
        `Failed to send WhatsApp message: ${res.status} ${err}`,
      );
    }
  }
