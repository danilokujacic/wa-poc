...
describe('WebhookSignatureGuard', () => {
  ...
  it('accepts a request signed with the correct app secret', () => {
    ...
    expect(
      guard.canActivate(
        contextWith({ 'x-hub-signature-256': signature }, rawBody),
      ),
    ).toBe(true);
  });
  ...
  it('rejects a request with an invalid signature', () => {
    ...
    expect(() =>
      guard.canActivate(
        contextWith({ 'x-hub-signature-256': 'sha256=wrong' }, rawBody),
      ),
  ...
  it('rejects a request with a missing signature header', () => {
    ...
    expect(() => guard.canActivate(contextWith({}, rawBody))).toThrow(
  ...
  it('rejects a request with no raw body captured', () => {
    ...
    expect(() =>
      guard.canActivate(
        contextWith({ 'x-hub-signature-256': signature }, undefined),
      ),
    ).toThrow(UnauthorizedException);
  });
  ...
  it('rejects when WHATSAPP_APP_SECRET is not configured', () => {
    ...
    expect(() =>
      guard.canActivate(
        contextWith({ 'x-hub-signature-256': 'sha256=anything' }, rawBody),
      ),
    ).toThrow(UnauthorizedException);
  });
});
