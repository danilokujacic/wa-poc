...
export class EmailConfirmationService {
  constructor(
    ...
    @InjectPinoLogger(EmailConfirmationService.name)
  ...
  async createAndSend(user: User): Promise<void> {
    ...
    this.logger.info(
      `Email confirmation for ${user.email} — slug: ${confirmation.slug}, code: ${confirmation.code}`,
    );
  ...
  async confirm(slug: string, code: string): Promise<void> {
    const confirmation =
      await this.emailConfirmationRepository.findValidBySlug(slug);
    if (!confirmation) {
    ...
    await this.dataSource.transaction(async (manager) => {
      await manager.update(User, confirmation.user.id, {
        emailConfirmed: true,
      });
