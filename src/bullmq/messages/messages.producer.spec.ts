...
describe("MessagesProducer", () => {
    beforeEach(() => {
        ...
        Test.createTestingModule({
            providers: [MessageBatchProducer,
                {
                    provide: Queue,
                    useValue: mockQueue,
                },
    ...
    it("should add a message and schedule a flush job", async () => {
        ...
        expect(mockRedis.set).toHaveBeenCalledWith(`wa:seen:${message.id}`, '1', 'EX', 3600, 'NX');
        expect(mockRedis.rpush).toHaveBeenCalledWith(`wa:buffer:${data.conversationKey}`, JSON.stringify(message));
        expect(mockRedis.expire).toHaveBeenCalledWith(`wa:buffer:${data.conversationKey}`, 3600);
        expect(mockQueue.add).toHaveBeenCalledWith('flush', data, expect.objectContaining({
    ...
    it("should reschedule an existing flush job", async () => {
        ...
        mockRedis.set.mockResolvedValue("OK");
        mockRedis.llen.mockResolvedValue(1);
        const mockExistingJob = { getState: jest.fn().mockResolvedValue('delayed'), remove: jest.fn() };
    ...
    it("should handle queue.add failure and retry with a new jobId", async () => {
        ...
        mockRedis.llen.mockResolvedValue(1);
        mockQueue.getJob.mockResolvedValue(null);
        mockQueue.add.mockRejectedValueOnce(new Error("Queue add failed")).mockResolvedValue({});
    ...
    it('should drain and flush messages correctly', async () => {
        ...
        mockMulti.exec.mockResolvedValue([[null, messages.map(m => JSON.stringify(m))], [null, 1]]);
    ...
    it('should return empty array if no messages to drain', async () => {
        ...
        mockMulti.exec.mockResolvedValue([[null, []], [null, 0]]);
