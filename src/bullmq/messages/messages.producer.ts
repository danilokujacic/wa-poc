...
export class MessageBatchProducer {
    ...
    async addMessage(
        ...
        try {
            ...
            this.logger.error(`Error buffering message ${message.id} for ${conversationKey}: ${err}`);
            throw err;
        }
        ...
        try {
            ...
            this.logger.error(`Error scheduling flush job for ${conversationKey}: ${err}`);
            throw err;
        }


        const pending = await this.redis.llen(listKey);
        this.logger.debug(`Buffered message ${message.id} for ${conversationKey}. Pending: ${pending}`);
    }
    ...
    async drain(conversationKey: string): Promise<StoredMessage[]> {
        ...
        try {
            ...
            this.logger.error(`Error draining messages for ${conversationKey}: ${err}`);
            return [];
        }
    }
}