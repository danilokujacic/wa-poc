...
describe('DeskMessageProducer', () => {
    ...
    it('enqueues a durable, retrying job', async () => {
        const producer = new DeskMessageProducer(mockQueue as any, mockLogger as any);
        mockQueue.add.mockResolvedValue({});

        await producer.enqueue(data);

        expect(mockQueue.add).toHaveBeenCalledWith('record', data, expect.objectContaining({
            attempts: 36,
    ...
    it('logs and rethrows if enqueueing itself fails', async () => {
        ...
        expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining('Error enqueuing desk message'));
    });
});
