...
describe('WaSendProcessor', () => {
    ...
    it('sends via WhatsApp, marks the message Sent, and broadcasts the status update', async () => {
        ...
        expect(mockSendText).toHaveBeenCalledWith('phone-id-1', '38269280401', 'On it!');
        expect(mockUpdate).toHaveBeenCalledWith('message-1', { deliveryStatus: MessageDeliveryStatus.SENT });
    ...
    it('lets a transient (5xx) failure propagate as-is so BullMQ retries', async () => {
        const mockSendText = jest.fn().mockRejectedValue(new WhatsappSendError(503, 'Service Unavailable'));
        const processor = new WaSendProcessor(
            { sendText: mockSendText } as any,
    ...
    it('lets a rate-limit (429) failure propagate as-is so BullMQ retries', async () => {
        const mockSendText = jest.fn().mockRejectedValue(new WhatsappSendError(429, 'Too Many Requests'));
        const processor = new WaSendProcessor(
            { sendText: mockSendText } as any,
            { update: jest.fn() } as any,
            { emitMessageStatusUpdated: jest.fn() } as any,
    ...
    it('fails fast (no retry) on a permanent failure like a bad/expired token', async () => {
        const mockSendText = jest.fn().mockRejectedValue(new WhatsappSendError(401, 'Authentication Error'));
        const processor = new WaSendProcessor(
            { sendText: mockSendText } as any,
            { update: jest.fn() } as any,
            { emitMessageStatusUpdated: jest.fn() } as any,
    ...
    it('marks the message Failed and broadcasts once retries are exhausted', async () => {
        ...
        await processor.onFailed(failedJob, new WhatsappSendError(503, 'Service Unavailable'));

        expect(mockUpdate).toHaveBeenCalledWith('message-1', { deliveryStatus: MessageDeliveryStatus.FAILED });
        expect(mockEmit).toHaveBeenCalledWith('resort-1', {
            conversationId: 'conv-1',
            messageId: 'message-1',
    ...
    it('marks the message Failed immediately on an UnrecoverableError, even on the first attempt', async () => {
        ...
        await processor.onFailed(failFastJob, new UnrecoverableError('Authentication Error'));
    ...
    it('does not mark the message Failed while retries remain', async () => {
        ...
        await processor.onFailed(retryingJob, new WhatsappSendError(503, 'Service Unavailable'));
