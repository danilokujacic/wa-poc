...
import {
  MESSAGE_SENDER,
  WhatsappSendError,
} from '../messages/message-sender.interface';
...
export class WaSendProcessor extends WorkerHost {
  ...
  async process(job: Job<WaSendJobData>): Promise<void> {
    ...
    this.logger.info(
      { traceId, messageId, attempt: job.attemptsMade + 1 },
    ...
    try {
      ...
      if (error instanceof WhatsappSendError && !isRetryable(error.status)) {
        this.logger.warn(
          { traceId, messageId },
          `Permanent send failure (status ${error.status}), not retrying`,
        );
      ...
      this.logger.warn(
        { traceId, messageId },
        `Transient send failure, will retry: ${error}`,
      );
      throw error;
    }
  ...
  async onFailed(
    job: Job<WaSendJobData> | undefined,
    error: Error,
  ): Promise<void> {
    if (!job) return;
  ...
  private async updateDeliveryStatus(
    ...
    this.logger.info(
      { traceId, messageId, deliveryStatus },
