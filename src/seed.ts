...
const FAQS: Array<{ question: string; answer: string }> = [
  {
    ...
    answer:
      'Sunset Bay Resort sits right on Paradise Beach in Budva, Montenegro, with direct waterfront access.',
  },
  {
  ...
  },
  {
    ...
    answer:
      'Yes, breakfast is included with every room and is served from 7:00 AM to 10:30 AM.',
  },
  {
    question: 'Do you have a swimming pool?',
    answer:
      'Yes, we have an outdoor infinity pool open from 8:00 AM to 8:00 PM, plus direct beach access.',
  },
  {
...
const FEATURES: Array<{
...
}> = [
  {
    name: 'Private Cabana',
    description:
      'A private beachfront cabana for two, with sun loungers and shade.',
    price: 49.99,
    quantity: 6,
    capacity: 2,
  ...
  },
  {
    ...
    description:
      'A double room with a private balcony overlooking Paradise Beach.',
    price: 129.0,
    quantity: 10,
    capacity: 2,
...
const CONTACTS: Array<{
...
}> = [
  {
    ...
    contact: '+382 69 123 456',
  },
  {
    contact_name: 'Reservations',
    type: ContactType.EMAIL,
    contact: 'reservations@sunsetbayresort.example.com',
  },
];
...
async function seed() {
  ...
  let resort = await resortRepo.findOne({
    where: { phoneNumber: RESORT_PHONE_NUMBER },
  });
  if (!resort) {
    resort = await resortRepo.save(
      resortRepo.create({
        name: 'Sunset Bay Resort',
        phoneNumber: RESORT_PHONE_NUMBER,
        ...RESORT_DETAILS,
      }),
    );
    ...
    console.log(
      `Resort "${resort.name}" (${resort.phoneNumber}) already exists`,
    );
  }

  let detailsChanged = false;
  if (!resort.address) {
    resort.address = RESORT_DETAILS.address;
    detailsChanged = true;
  }
  if (!resort.latitude) {
    resort.latitude = RESORT_DETAILS.latitude;
    detailsChanged = true;
  }
  if (!resort.longitude) {
    resort.longitude = RESORT_DETAILS.longitude;
    detailsChanged = true;
  }
  if (!resort.startWorkingHours) {
    resort.startWorkingHours = RESORT_DETAILS.startWorkingHours;
    detailsChanged = true;
  }
  if (!resort.endWorkingHours) {
    resort.endWorkingHours = RESORT_DETAILS.endWorkingHours;
    detailsChanged = true;
  }
  if (!resort.website) {
    resort.website = RESORT_DETAILS.website;
    detailsChanged = true;
  }
  if (detailsChanged) {
    resort = await resortRepo.save(resort);
    console.log(
      '  + Filled in missing resort details (address/coordinates/hours/website)',
    );
  }
  ...
  for (const feature of FEATURES) {
    ...
    if (!existing) {
      existing = await featureRepo.save(
        featureRepo.create({ ...feature, resort }),
      );
