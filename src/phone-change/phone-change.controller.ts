...
export class PhoneChangeController {
    ...
    @ApiOperation({ summary: 'Request a phone number change for this resort — only its owner may do this; requires manual approval, max once per week' })
    @ApiParam({ name: 'resortId', type: String })
    async create(@Param('resortId') resortId: string, @Body() dto: CreatePhoneChangeDto) {
        const phoneChange = await this.phoneChangeService.create(resortId, dto.newPhoneNumber);
