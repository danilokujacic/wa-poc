...
export class PhoneChangeService {
  ...
  async create(resortId: string, newPhoneNumber: string): Promise<PhoneChange> {
    ...
    const recent = await this.phoneChangeRepository.findRecentForResort(
      resortId,
      oneWeekAgo,
    );
    if (recent) {
      throw new ConflictException(
        'A phone number change request was already submitted for this resort in the past week',
      );
    }
    ...
    if (adminEmail) {
      await this.mailService.sendMail({
        ...
        html:
          `<p>Resort <strong>${resort.name}</strong> (${resort.id}) requested a phone number change.</p>` +
