...
describe('ReservationController', () => {
    ...
    const feature = { id: 'feature-1', name: 'Cabana', description: null, price: 49.99, quantity: 5, capacity: 2, images: null };
    const reservation = {
        id: 'reservation-1',
    ...
    it('delegates getAvailabilityForAllFeatures to the service', async () => {
        service.getAvailabilityForAllFeatures.mockResolvedValue([{ featureId: 'feature-1', name: 'Cabana', availability: 3 }]);

        await controller.getAvailabilityForAllFeatures('resort-1');

        expect(service.getAvailabilityForAllFeatures).toHaveBeenCalledWith('resort-1');
    });
    ...
    it('delegates getAvailability to the service', async () => {
        ...
        expect(service.getAvailability).toHaveBeenCalledWith('resort-1', 'feature-1');
    ...
    it('delegates updateStatus to the service', async () => {
        ...
        service.updateStatus.mockResolvedValue({ ...reservation, status: ReservationStatus.ACCEPTED });

        await controller.updateStatus('resort-1', 'reservation-1', dto);

        expect(service.updateStatus).toHaveBeenCalledWith('resort-1', 'reservation-1', ReservationStatus.ACCEPTED);
    });
