...
export class ReservationController {
    ...
    async create(@Param('resortId') resortId: string, @Body() dto: CreateReservationDto) {
    ...
    @ApiParam({ name: 'resortId', type: String })
    @ApiQuery({ name: 'from', required: false, type: String, description: 'Start date filter (ISO string)' })
    @ApiQuery({ name: 'to', required: false, type: String, description: 'End date filter (ISO string)' })
    @ApiQuery({ name: 'status', required: false, type: String, description: "Reservation status, or 'ALL'" })
    @ApiQuery({ name: 'phoneNumber', required: false, type: String, description: 'Filter by phone number (partial match)' })
    async findAll(
        @Param('resortId') resortId: string,
        @Query(new LenientValidationPipe(FindReservationsQueryDto)) query: FindReservationsQueryDto,
    ) {
    ...
    @Get('availability')
    @ApiOperation({ summary: "Get availability for all of a resort's features — its owner or employees may do this" })
    @ApiParam({ name: 'resortId', type: String })
    async getAvailabilityForAllFeatures(@Param('resortId') resortId: string) {
        const results = await this.reservationService.getAvailabilityForAllFeatures(resortId);
        return results.map((result) => FeatureAvailabilityResponseDto.fromResult(result));
    }

    @Get('availability/:featureId')
    @ApiOperation({ summary: "Get availability for one of a resort's features — its owner or employees may do this" })
    @ApiParam({ name: 'resortId', type: String })
    @ApiParam({ name: 'featureId', type: String })
    getAvailability(@Param('resortId') resortId: string, @Param('featureId') featureId: string) {
    ...
    @Patch(':id/status')
    @UsePipes(new ValidationPipe({ transform: true, whitelist: true, forbidNonWhitelisted: true }))
    @ApiOperation({ summary: "Update the status of one of a resort's reservations — its owner or employees may do this" })
    @ApiParam({ name: 'resortId', type: String })
    @ApiParam({ name: 'id', type: String })
    async updateStatus(
        @Param('resortId') resortId: string,
        ...
        const reservation = await this.reservationService.updateStatus(resortId, id, dto.status);
        return ReservationResponseDto.fromEntity(reservation);
    }
