...
describe('ResortUserService', () => {
  ...
  it('throws when getting one user that does not belong to the resort', async () => {
    ...
    await expect(service.findOne('resort-1', 'user-1')).rejects.toThrow(
      NotFoundException,
    );
  });
  ...
  it('rejects replacing a user with an email already taken by someone else', async () => {
    userRepository.findOne.mockResolvedValue({
      id: 'user-1',
      name: 'Old',
      email: 'old@example.com',
    ...
    await expect(
      service.replace('resort-1', 'user-1', {
        name: 'New Name',
        email: 'new@example.com',
        password: 'password123',
      }),
  ...
  it('partially updates a user', async () => {
    ...
    const result = await service.update('resort-1', 'user-1', {
      name: 'New Name',
    });
  ...
  it('allows updating a user to keep its own current email unchanged', async () => {
    userRepository.findOne.mockResolvedValue({
      id: 'user-1',
      name: 'Old',
      email: 'old@example.com',
  ...
  it('rejects updating a user to an email already taken by someone else', async () => {
    userRepository.findOne.mockResolvedValue({
      id: 'user-1',
      name: 'Old',
      email: 'old@example.com',
    ...
    await expect(
      service.update('resort-1', 'user-1', { email: 'taken@example.com' }),
    ).rejects.toThrow(ConflictException);
    expect(userRepository.save).not.toHaveBeenCalled();
  });
