...
export class ConversationRepository extends Repository<Conversation> {
    ...
    async findOrCreate(resortId: string, guestPhoneNumber: string): Promise<Conversation> {
        ...
        try {
            return await this.save(
                this.create({ resort: { id: resortId } as Conversation['resort'], guestPhoneNumber }),
            );
        } catch (error) {
    ...
    async isHumanHandled(resortId: string, guestPhoneNumber: string): Promise<boolean> {
