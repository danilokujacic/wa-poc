...
describe('ResortService', () => {
    ...
    let repository: { create: jest.Mock; save: jest.Mock; find: jest.Mock; findOneBy: jest.Mock; remove: jest.Mock };
    ...
    it('creates a resort for an owner with no resort yet', async () => {
        ...
        expect(userRepository.save).toHaveBeenCalledWith(expect.objectContaining({ resort: dto }));
    ...
    it('rejects creation when the owner is not found', async () => {
        ...
        await expect(service.create({ name: 'Sunset Bay', phoneNumber: '123' }, 'missing')).rejects.toThrow(
            NotFoundException,
        );
    ...
    it('rejects creation when the owner already has a resort', async () => {
        userRepository.findOne.mockResolvedValue({ id: 'user-1', resort: { id: 'resort-1' } });

        await expect(service.create({ name: 'Sunset Bay', phoneNumber: '123' }, 'user-1')).rejects.toThrow(
            ConflictException,
        );
        expect(repository.create).not.toHaveBeenCalled();
    });
    ...
    it('updates a resort', async () => {
        ...
        expect(repository.save).toHaveBeenCalledWith({ ...resort, name: 'New Name' });
