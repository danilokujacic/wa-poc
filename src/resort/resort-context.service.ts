...
export class ResortContextService {
  constructor(
    ...
    @InjectPinoLogger(ResortContextService.name)
  ...
  async warm(conversationKey: string, phoneNumberId: string): Promise<void> {
    try {
      ...
      this.logger.error(
        `Error warming resort context for ${conversationKey}: ${err}`,
      );
    }
  }
  ...
  async get(
    ...
    const resort =
      await this.resortRepository.findByPhoneNumberWithCore(phoneNumberId);
    if (resort) {
      await this.redis.set(
        cacheKey,
        JSON.stringify(resort),
        'EX',
        RESORT_CONTEXT_CACHE_TTL_SECONDS,
      );
    }
    return resort;
  }
