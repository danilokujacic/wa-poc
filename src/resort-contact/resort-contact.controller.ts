...
export class ResortContactController {
  ...
  @ApiOperation({
    summary: 'Create a contact for this resort — only its owner may do this',
  })
  @ApiParam({ name: 'resortId', type: String })
  async create(
    @Param('resortId') resortId: string,
    @Body() dto: CreateResortContactDto,
  ) {
  ...
  @ApiOperation({
    summary: "List a resort's contacts — its owner or employees may do this",
  })
  @ApiParam({ name: 'resortId', type: String })
  @ApiQuery({
    name: 'search',
  ...
  async findAll(
    @Param('resortId') resortId: string,
    @Query(new LenientValidationPipe(FindResortContactsQueryDto))
    query: FindResortContactsQueryDto,
  ) {
    const contacts = await this.resortContactService.findAll(resortId, query);
    return contacts.map((contact) =>
      ResortContactResponseDto.fromEntity(contact),
    );
  }
  ...
  @ApiOperation({
    summary:
      "Get one of a resort's contacts — its owner or employees may do this",
  })
  @ApiParam({ name: 'resortId', type: String })
  ...
  @ApiOperation({
    summary: "Update one of a resort's contacts — only its owner may do this",
  })
  @ApiParam({ name: 'resortId', type: String })
  @ApiParam({ name: 'id', type: String })
  async update(
  ...
  @UseGuards(ResortOwnerGuard)
  @ApiOperation({
    summary: "Delete one of a resort's contacts — only its owner may do this",
  })
  @ApiParam({ name: 'resortId', type: String })
  @ApiParam({ name: 'id', type: String })
