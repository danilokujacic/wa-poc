...
export class ResortContactService {
  constructor(
    private readonly resortContactRepository: ResortContactRepository,
  ) {}

  create(
    resortId: string,
    createResortContactDto: CreateResortContactDto,
  ): Promise<ResortContact> {
  ...
  findAll(
    resortId: string,
    { search = '' }: FindResortContactsQueryDto,
  ): Promise<ResortContact[]> {
    const where:
      FindOptionsWhere<ResortContact> | FindOptionsWhere<ResortContact>[] =
      search
        ? [
  ...
  async findOne(resortId: string, id: string): Promise<ResortContact> {
    ...
    if (!contact) {
      throw new NotFoundException(
        `Contact with id ${id} not found for resort ${resortId}`,
      );
    }
    return contact;
  }
  ...
  async update(
    resortId: string,
    id: string,
    updateResortContactDto: UpdateResortContactDto,
  ): Promise<ResortContact> {
